...
ATEventManagerS800::DrawPIDFull()
{
    ...
    fPIDFull = new TH2F("PIDFull","PIDFull",500,-100,0,300,250,280);
...
ATEventManagerS800::DrawPID2Full()
{
    ...
    fPID2Full = new TH2F("PID2Full","PID2Full",500,-100,0,1500,0,750);  
    //fLvsTheta->SetMarkerStyle(22);
    //fLvsTheta->SetMarkerColor(kRed);
...
//----------- New 10/01 ------------------------------------- 
      if(CondMTDCXfObj)fPIDFull->Fill(S800_timeObjSelect,XfObj_tof);
      if(CondMTDCObj)fPID2Full->Fill(S800_timeObjSelect,S800_ICSum);
      //if(std::isnan(S800_tofCorr)==0) fPIDFull->Fill(XfObj_tof,S800_timeObj);
      //if(std::isnan(S800_tofCorr)==0) fPID2Full->Fill(S800_ICSum,S800_timeObj);
//----------- New 10/01 ------------------------------------- 
